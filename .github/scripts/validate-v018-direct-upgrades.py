...
Version-specific v0.18.0 executor. The v0.17.0 executor remains unchanged so
its retained release evidence keeps its original execution authority.
...
ORIGINS = {
    ...
    "v0.17.0": ("66a731f04695386416d7eb92b8a4d49ea8daaf4b118d8c93825764315c982b1a", "f262df5acb224ef260f855b08f9b8bb130ec4f5d280209af6475c4b1a06c4059"),
}
EVIDENCE = ".dev/decisions/direct-v018-fixture.md"
...
def fixture_customization(identity):
    return {"id": "CUST-UPGRADE-PRESERVATION", "subject": {"kind": "contract", "id": "target-upgrade-preservation"},
            ...
            "active_context_audit": {"assessment_id": "ASM-20260912-14-a7c", "status": "verified", "evidence": EVIDENCE},
            "incoming": {"version": identity["version"], "status": "absent", "evidence": EVIDENCE}, "disposition": "retain",
            "post_upgrade_audit": {"assessment_id": "ASM-20260912-14-a7c", "status": "verified", "evidence": EVIDENCE},
...
if (root/'.git/fail-target-validation').exists():
    raise SystemExit(17)
expected=json.loads((root/'.dev/validation/expected-v018.json').read_bytes())
...
def seed_target(previous, incoming, target, customized, apply, provenance, rules, logs):
    ...
    write_json(target / ".dev/validation/expected-v018.json", {"managed": managed, "preserved": preserved, "removed": removed})
...
def execute_case(origin, previous, incoming, output, customized, recovery, apply, provenance, rules):
    ...
    candidate["last_migration"] = {"status": "completed", "from_version": origin, "to_version": "v0.18.0", "completed_at": stamp(), "evidence": EVIDENCE}
    candidate_ledger = deepcopy(ledger)
    for entry in candidate_ledger["customizations"]:
        entry["incoming"] = {"version": "v0.18.0", "status": "absent", "evidence": EVIDENCE}
