#!/usr/bin/env python3
"""local-backlog.fs 0.1.0. Owned standalone JSON-request tool; no cross-skill imports."""
...
PACKAGE = 'local-backlog'
ID_PREFIX = 'work-'
SUFFIX = '.work-item.json'
ROLE = 'local-backlog.record'
OPERATIONS = ('explain', 'create', 'inspect', 'query', 'revise', 'transition', 'render')
ALL_OPERATIONS = ['explain', 'create', 'inspect', 'query', 'revise', 'transition', 'render']
AUTHORED = ('title', 'summary', 'acceptance', 'references')
VIEW_FIELDS = ('id', 'title', 'summary', 'state', 'state_reason', 'acceptance', 'completion_evidence', 'references')
MEMBERS = ['SKILL.md',
 'skill-package.yaml',
...
DEFAULTS = {'store': {'kind': 'filesystem', 'root': 'notes/work-items', 'tracking': 'tracked'},
 'template': {'origin': 'package', 'path': 'templates/work-item.md'}}
TOOLS = {'local-backlog.fs': ('scripts/local_backlog.py',
                      ['explain', 'create', 'inspect', 'query', 'revise', 'transition', 'render'],
                      'references/operations.md')}
ROLES = [{'role': 'local-backlog.record',
  'owner': 'project',
  'schema': 'local-backlog.record@1.0.0',
  'read_schemas': ['local-backlog.record@1.0.0'],
  'store_binding': 'local-backlog.store',
  'identity': 'work-<32 lowercase hex digits>',
  'filename': '<id>.work-item.json',
  'read_operations': ['inspect', 'query', 'render'],
  'write_operations': ['create', 'revise', 'transition']},
...
RUNTIME = [{'id': 'skill-instruction-reader',
  ...
  'for_operations': ['explain', 'create', 'inspect', 'query', 'revise', 'transition', 'render'],
 ...
 {'id': 'jsonschema',
  'version': '>=4.18,<5',
  'for_operations': ['create', 'inspect', 'query', 'revise', 'transition', 'render'],
  'on_missing': 'unavailable'},
 {'id': 'filesystem',
  ...
  'on_missing': 'unavailable'},
 {'id': 'git',
  'requirement': 'Conditional: selected local-config ignore checks',
  'for_operations': ['explain', 'create', 'inspect', 'query', 'revise', 'transition', 'render'],
  'on_missing': 'unavailable'}]
SCHEMA_PATH = 'schemas/local-backlog-record.schema.json'
...
class Binding:
    ...
    def record_path(self, ref):
        ...
        if ref["role"] != "local-backlog.record" or type(ref["id"]) is not str or not ID.fullmatch(ref["id"]):
            fail("reference", "Invalid store-scoped record reference.")
        return safe_path(ref["id"] + ".work-item.json", self.store, relative_only=True)
...
def query(binding, text):
    ...
    for path in selected:
        ...
        try:
            ...
            record_id = path.name.removesuffix(".work-item.json")
...
class Writer:
    ...
    def __init__(self, binding):
        ...
        self.lock = binding.store / ".local-backlog-write.lock"
        self.temp = binding.store / (".local-backlog-" + self.token + ".tmp")
        self.owned = {}
