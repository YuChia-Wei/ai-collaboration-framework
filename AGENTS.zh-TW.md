...
- 當操作可能跨越 shell、sandbox、host、WSL、container、CI、connector、credential 或 external-task 邊界時，遵循 `.ai/assets/shared/ENVIRONMENT-EXECUTION-ROUTING-CONTRACT.md`。
- 僅從 `.dev/ai-context/local/environment-execution-routing.yaml` 讀取選用的 per-clone binding。此檔案必須由 `.gitignore` 的 `/.dev/ai-context/local/` 規則涵蓋、保持 untracked 與 unstaged、不含秘密，且不得成為 package 或 provenance truth。
- 不得隱含建立或更新該 binding。當 bounded diagnosis 找到能成功完成所要求操作的穩定路由時，先完成並驗證操作，再詢問使用者是否要在本機保存最小路由資訊。
