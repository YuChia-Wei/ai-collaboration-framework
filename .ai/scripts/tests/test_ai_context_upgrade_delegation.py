...
class AiContextUpgradeDelegationGwtTests(unittest.TestCase):
    ...
    def test_gwt_006_given_fallback_without_exact_evidence_when_validated_then_fails_closed(self) -> None:
        ...
        try:
            ...
            record["fallbacks"] = [
                {
                    "scope": "role-evaluation",
                    "disposition": "root-sequential",
                    "trigger": "verified worker support is unavailable",
                    "observed": "delegation-support-unavailable",
                    "selected": "root-sequential",
                    "owner_consent": "explicit-owner-choice",
            ...
            fixture.write_yaml(self.template_path, record)
            _, errors = fixture.validate()
            self.assert_error(errors, "missing required fields")
    ...
    def test_gwt_010_given_none_mode_when_root_sequential_stages_match_then_passes(self) -> None:
        ...
        try:
            ...
            record["selection"]["max_concurrent_workers"] = 0
            record["selection"]["eligible_role_asset_ids"] = []
            record["execution_path"]["kind"] = "root-sequential"
            fixture.write_yaml(self.template_path, record)
            count, errors = fixture.validate()
            self.assertEqual([], errors)
            ...
            record["fallbacks"] = [
                {
                    ...
                    "disposition": "root-sequential",
                    "trigger": "verified worker support is unavailable",
                    "requested": "delegation-evaluation",
                    "observed": "delegation-support-unavailable",
                    "selected": "root-sequential",
                    "owner_consent": "explicit-owner-choice",
    ...
    def test_gwt_013_given_none_mode_with_an_eligible_role_when_validated_then_fails_closed(self) -> None:
        fixture = DelegationRunContractFixture()
        try:
            record = fixture.read_yaml(self.template_path)
            record["selection"]["mode"] = "none"
            ...
            fixture.write_yaml(self.template_path, record)
            _, errors = fixture.validate()
            self.assert_error(errors, "must retain the exact mode-eligible roles")
        finally:
            fixture.close()

    def test_gwt_014_given_analysis_only_with_a_mechanical_role_when_validated_then_fails_closed(self) -> None:
        fixture = DelegationRunContractFixture()
        try:
            record = fixture.read_yaml(self.template_path)
            record["selection"]["mode"] = "analysis-only"
    ...
    def test_gwt_015_given_full_recommended_exceeds_two_workers_when_validated_then_fails_closed(self) -> None:
        fixture = DelegationRunContractFixture()
        try:
            record = fixture.read_yaml(self.template_path)
            record["selection"]["max_concurrent_workers"] = 3
            fixture.write_yaml(self.template_path, record)
            _, errors = fixture.validate()
            self.assert_error(errors, "must retain the exact mode concurrency")
        finally:
            fixture.close()

    def test_gwt_016_given_root_fallback_selected_field_mismatches_when_validated_then_fails_closed(self) -> None:
        fixture = DelegationRunContractFixture()
        try:
            record = fixture.read_yaml(self.template_path)
            record["execution_path"]["kind"] = "root-sequential"
            record["fallbacks"] = [
                {
                    ...
                    "requested": "delegation-evaluation",
                    "observed": "delegation-support-unavailable",
                    "selected": "delegation-evaluation",
                    "owner_consent": "explicit-owner-choice",
                    "authorization_evidence": ["owner-selected fallback"],
                    "evidence_refs": ["workflow-task#worker-unavailable"],
                    "canonical_stage_ids": record["canonical_stage_ids"],
            ...
            fixture.write_yaml(self.template_path, record)
            _, errors = fixture.validate()
            self.assert_error(errors, "selected must be 'root-sequential'")
        finally:
            fixture.close()

    def test_gwt_017_given_exact_independent_auditor_fallback_when_validated_then_passes(self) -> None:
        fixture = DelegationRunContractFixture()
        try:
            record = fixture.read_yaml(self.template_path)
            record["fallbacks"] = [
                {
                    "scope": "terminal-independent-audit",
                    "disposition": "fresh-independent-context",
                    "trigger": "primary independent auditor is unavailable",
                    "requested": "terminal-independent-audit",
                    "observed": "primary-independent-auditor-unavailable",
                    "selected": "fresh-independent-context",
                    "owner_consent": "explicit-owner-choice",
                    "authorization_evidence": ["owner-selected auditor fallback"],
                    "evidence_refs": ["workflow-task#auditor-unavailable"],
                    "canonical_stage_ids": [
    ...
    def test_gwt_018_given_retained_workflow_record_when_reusable_validator_runs_then_passes(self) -> None:
        errors: list[str] = []
        passed = VALIDATOR.validate_upg004_delegation_run_record_path(
            errors,
            root=REPO_ROOT,
            record_path=Path(
                ".dev/workflows/2026-08-21-upg-004-delegation-preflight/"
                "evidence/delegation-run.yaml"
            ),
        )
