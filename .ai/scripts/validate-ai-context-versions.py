...
RETAINED_UPGRADE_TEST_SOURCES = {
    "v0.14.0": ["v0.6.0", "v0.9.0", "v0.13.0"],
}
...
def validate_upgrade_test_horizon(
    ...
    """Bind routine upgrade evidence to the governed source horizon."""
    ...
    for index, (current_key, version, path, data) in enumerate(governed):
        ...
        if compatibility.get("automatic_upgrade_sources") != expected_sources:
            errors.append(
                ...
                f"the governed package sources {expected_sources} in numeric order in "
        ...
        if (
            ...
            and compatibility.get("minimum_source_version") != expected_minimum
        ):
            errors.append(
                f"{path}: breaking release {version} is a migration checkpoint and "
                f"compatibility.minimum_source_version must be {expected_minimum}"
            )
