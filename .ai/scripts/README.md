...
`generate-runtime-skill-entries.py` is the portable deterministic pilot generator for
the `local-change-implementer` and `code-reviewer` runtime `SKILL.md` entries.
Its sole execution source is each selected canonical `skill.yaml` `runtime_entry`;
the generated frontmatter is discovery metadata, not rule authority. Regenerate
with an absolute repository root and run `--check` to prove exact Codex/Claude
parity and canonical-source provenance. It normalizes source text to Git LF
bytes before hashing. During portable package assembly, `ai_context_package.py`
uses the same renderer against the final projected `skill.yaml` payload bytes,
so an installed wrapper has projected authority and provenance without a
downstream regeneration step. The generator uses the already governed
`PyYAML==6.0.3` dependency declared in `python-entrypoints.json`, root
`requirements.txt`, and the package envelope mirror.
