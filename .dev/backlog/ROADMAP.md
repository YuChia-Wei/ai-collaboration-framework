...
| `v0.8.0` | `planning` | yes | Preserve the completed skill-owned Python automation colocation and fail-closed Python prerequisite diagnostics across every governed validator entrypoint. | `SKILL-002` and `TOOL-002` are merged, resolved, and awaiting publication. Release candidate creation and publication require separate owner authorization. | [`2026-07-30-skill-script-colocation`](../workflows/2026-07-30-skill-script-colocation/workflow.yaml), [`2026-08-02-python-prerequisite-diagnostics`](../workflows/2026-08-02-python-prerequisite-diagnostics/workflow.yaml) |
...
| `v0.8.0` | `SKILL-002`, `TOOL-002` (both resolved; awaiting publication) | None currently selected. Additional scope still requires an explicit owner roadmap decision rather than silent inclusion. | `SKILL-002` completed through PR #66 and Issue #65. `TOOL-002` completed through PR #78, merge `2070e44`, independent verification, Issue #77, and Project #3 Done read-back. Release preparation remains unauthorized. |
...
- `v0.8.0`: `SKILL-002` owns compatibility-safe colocation of single-owner
  ...
  resolved, and awaiting publication. No release candidate or publication is
  authorized.
...
Retain the completed `SKILL-002` and `TOOL-002` evidence. Do not create a
v0.8.0 release candidate, tag, GitHub Release, or published package until the
owner separately authorizes release preparation. Keep
