...
| `v0.8.0` | `planning` | yes | Co-locate skill-owned Python automation with canonical skills while preserving shared tooling and published compatibility routes. | Local `SKILL-002` validation is complete; merge and read back the implementation on `main` before resolution. Release candidate creation and publication require separate owner authorization. | [`2026-07-30-skill-script-colocation`](../workflows/2026-07-30-skill-script-colocation/workflow.yaml) |
...
| `v0.8.0` | `SKILL-002` | None currently selected. New scope requires an explicit owner roadmap decision rather than silent inclusion. | v0.7.0 is published; Issue #65 is in Verification pending merge read-back. Release preparation remains unauthorized. |
...
- `v0.8.0`: `SKILL-002` owns compatibility-safe colocation of single-owner
  Python automation and contract tests under canonical skill directories. It is
  locally validated but pending integration into `main`; no release candidate or publication is authorized.
...
Merge and read back the validated `SKILL-002` implementation on `main`, then
resolve the item and close Issue #65. Do not create a v0.8.0 release candidate,
tag, GitHub Release, or published package until the owner separately authorizes
release preparation. Keep
