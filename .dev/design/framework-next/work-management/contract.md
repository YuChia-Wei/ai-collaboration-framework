# Work management contract proposal

This is intended product behavior for coordinator reconciliation, not evidence of implementation.
...
## 2. Configuration proposal (WM-C1)
...
**Coordinator decision:** recommend retaining config_version 1 while deliberately generalizing the envelope to namespaced JSON objects: validate root keys/version, namespace-ID grammar and object shape; validate only the selected namespace strictly; leave unselected sibling objects inert. They do not prove installed capabilities or valid settings for another owner. Unknown keys inside the selected namespace still fail. This changes the earlier unknown-namespace rejection and must be adopted coherently for Lesson and P3-A consumers. Alternative: coordinator-owned explicit namespace registration. The coordinator selects this behavior and version decision before implementation; no consumer silently invents it.
...
**WM-C3:** preflight/post-read are optimistic observations, not atomic compare-and-swap. [GitHub conditional request guidance](https://docs.github.com/en/rest/using-the-rest-api/best-practices-for-using-the-rest-api#use-conditional-requests-if-appropriate) does not establish conditional PATCH support here. A concurrent writer can be overwritten between read/write without detectable post-read evidence. Recommend explicitly coordinated single-writer use; callers requiring server-enforced CAS receive unsupported. Alternative: coordinator keeps provider-update unsupported until a proven conditional-write adapter is selected. Do not invent If-Match guarantees.
