# REL-v0.18.0 — Skill Usability And Transactional Messaging

This release brings private role resources beside their owning skills and makes
routine collaboration proportionate to the operation's scope. It also defines
consistent .NET inbox/outbox transaction guidance across design, implementation,
review and test selection.

## Highlights

- Seventeen private role directories belong to `code-reviewer`,
  `slice-implementer` and `problem-frame-author`. Shared roles retain their
  existing canonical home and stable role identifiers.
...
Private role paths change. Reconcile target-local role customizations before
retiring their old paths. Target-owned technology choices, project facts and
