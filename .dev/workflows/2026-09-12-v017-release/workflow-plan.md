...
- `workflow_id`: `2026-09-12-v017-release`
- `workflow_kind`: `release-publication`
...
Use the owner-selected RAM-backed locations through explicit local execution settings and fresh write/cleanup preflight. Keep machine-specific paths only under ignored local artifacts. Classified disposable fixtures may consume AI_CONTEXT_TEST_TMP_ROOT; storage/platform semantics must remain faithful. No silent fallback and no global environment changes. Bind long-running canonical commands to clean immutable snapshots and one external terminal callback.
...
- Candidate: the admitted assets built at `346bbc14e16d7219012de846267febd3a22cf1b6` are superseded for preparation because this repair changes payload bytes. Preserve those exact assets, build and admit a new immutable candidate after the repair commit, and obtain independent review before another actual dispatch.
- Next: finish repair review and candidate admission, then obtain the applicable retry authority, verify all nine actual cases, assemble fresh direct routes, and run candidate/critical gates and final independent review.
...
The owner authorized local release preparation and explicitly approved dispatch four after the separate fourth-dispatch question. The packet remains bound to the reviewed preparation commit, unchanged candidate archive, all eight verified ZIP/checksum inputs and the prior failure digest. Do not treat an earlier authorization or a parent-created retry record as authority to bypass a new required checkpoint. Preserve failures, pre-start interruption and setup corrections separately from actual-case evidence. No further automatic retry or publication action is authorized by this checkpoint.
