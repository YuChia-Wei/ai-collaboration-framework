...
8. Owner-authorized v0.9.0 release-allocation addendum — in progress under `SAR94-003`. Create one canonical `SAG-002` release item aggregating #118 and dependent #119, retain Proposal #94 as provenance, reconcile ROADMAP and provider mappings, then project only representative Issue #118 into the Project `待發布` state. Packaging, tag, GitHub Release, and publication remain excluded.
...
- Current task: `SAR94-003` is in progress on `codex/2026-08-05-sub-agent-reachability-cont-03` from `main@2aa61dd7a33916782170b8e3cc6de2e098c555d8`.
- Exact next action: commit the canonical `SAG-002`/ROADMAP/provider-contract checkpoint, then perform the external Issue/Project reconciliation, create the provider receipt, rely only on hosted CI, and integrate with the already selected merge-commit topology after all applicable gates pass.
...
| 4 | `codex/2026-08-05-sub-agent-reachability-cont-03` | `main@2aa61dd7a33916782170b8e3cc6de2e098c555d8` | active release-allocation addendum | pending canonical checkpoint | local branch | `2026-08-05T10:00:52+08:00` | owner explicitly confirmed v0.9.0 allocation after implementation closeout; external Project reconciliation requires durable ordered state | commit canonical allocation first, then synchronize provider state and open the required merge-commit PR without publication work |
