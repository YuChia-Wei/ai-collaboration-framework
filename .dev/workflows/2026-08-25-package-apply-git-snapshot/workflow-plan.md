...
- Current stage: implementation and same-commit aggregate validation are complete. Exact-head audits correctly rejected repository-local policy drift at `7a6f20a0`, config-selection drift at `ee55880b`, dormant include-target drift at `ad0b38a0`, relative local-origin resolution at `01f73f25`, and an in-capture config TOCTOU at `7a1a457a`. The current repair establishes config identity before semantic attribute/ignore reads and closes the snapshot with an exact config read-back; GWT-093 reproduces the audit timing and fails closed before target mutation. A fresh exact-head audit remains.
