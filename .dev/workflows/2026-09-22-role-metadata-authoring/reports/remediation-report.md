...
`role.migrate` 必須明示 from_version=1.0、to_version=1.1，且 wrapper_targets 原本已是空清單，adapter_metadata 原本不存在或為空 mapping。只變更這兩個版本欄位。1.0 只是 migration input，不會因此成為現行 canonical admission；其他版本、promoted adapters、已搬移而失效的參照都需要 owner reconciliation。兩種操作目前均限定動態角色。
...
尚待固定實作 commit 的獨立審查與 portable package smoke。測試內使用 disposable fixtures；沒有對現行 repository 的角色資料執行 migration，也不代表實際 runtime invocation、release、downstream adoption 或 hosted CI。未量測 token／總工時節省。
...
待獨立 auditor 檢查固定版本。本文是實作者說明，不代替 reviewer 結論。
...
目前 ROLE-001 完成實作與 focused checks，ROLE-002 進入固定版本驗證。owner 已授權本機合併，預定保留 implementation 與 evidence-intake 兩個 checkpoint。Issue 318 維持 open；沒有 push、PR、Issue/Project closure 或 release。最終合併與 current-subject binding 必須由完成時的 live read-back 證明。
