...
- Current stage: S3 is active from clean integrated main `bf8ad9c624ffc2154722dfb266c9090c72e4ac5f`; the Phase-A release scaffold and fail-closed release-state contract pass 36/36 focused tests. Immutable builds at `803a0771ada510a9c5faf630a36a096d74117a64`, `7680190bf673c403ffbb970c75d397f91606872c`, `a66f1901584cc2dba89065273e998bdc6f1b3c20`, and exact clean `2e6c5906882705e00b80f656c22c2925ed75c63a` failed before package output on, respectively, source ordering, a portable link to a source-only policy, the canonical code-reviewer framework-source authority reference, and missing target-owned `.dev/ai-context/provenance.yaml` referenced by code-reviewer. All failures remain retained; the fourth produced no output directory, package, matrix, or routes. The current uncommitted repair makes only exact canonical legacy source/provenance/customizations/effective-rules/packets/local/validation.local inputs eligible as absent target-owned runtime references, leaves `.dev/ai-context/unknown.yaml` failing, and has focused positive-provenance/negative-unknown GWT plus AST/YAML/scoped-diff evidence. No immutable builder success is claimed before the new immutable commit and rebuild.
...
- Current next gate: Preserve all four immutable builder failures and the real-manifest probe failure, commit the corrected portable-skill projection plus exact target-owned runtime-reference allowlist at an immutable head, then rebuild Phase-B without treating a synthetic GWT as package, matrix, or route proof.
