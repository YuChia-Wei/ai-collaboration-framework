...
- `updated_at`: `2026-08-02T19:21:46+08:00`
...
| `D-003` | Diagnostic output and exit contract umbrella | decomposed | Resolve human/machine fields and channels through `D-003A`, then numeric exit-code compatibility through `D-003B`. | Prevents a structured-output improvement from silently changing existing shell/CI control flow. |
...
| `D-003B` | Numeric exit-code compatibility | pending |  | Determines whether existing per-entrypoint nonzero codes remain compatible or prerequisite failures adopt one new shared code, including native-launcher behavior and migration evidence. |
...
| --- | --- | --- |
| Resolved atomic decisions (17) | `D-001`, `D-002A`, `D-002B`, `D-002C`, `D-003A`, `D-004A`, `D-004B`, `D-006A`, `D-006B`, `D-009`, `D-TR-001`, `D-010A`, `D-010B1`, `D-010B2`, `D-010C`, `D-010D`, `D-010E` | Production-entrypoint coverage, no-mutation failure, deterministic interpreter trust/discovery, stable native launchers with replaceable discovery, human/JSON diagnostic projections, exact per-entrypoint dependency profiles, documentation-only recovery, workflow ownership, packaged-but-unselected skill self-tests, retained v0.8.0 Python runtime, future translation routing, routine-switch scope, local and CI policy, compatibility-safe result truth, and bounded Agent attempts are approved. |
| Decomposed umbrellas (2) | `D-002`, `D-003` | `D-002` organizes its now-resolved runtime subdecisions. `D-003A` is resolved while numeric exit compatibility remains open in `D-003B`. Neither umbrella independently authorizes implementation; `D-004` and `D-010` are fully resolved through their control chains. |
| Pending atomic decisions (5) | `D-003B`, `D-005`, `D-006`, `D-007`, `D-008` | Numeric exit compatibility, pre-mutation proof, remaining script ownership, tests, and migration communication remain open. |
...
1. Observable contract and proof: `D-003B`, `D-005`, and `D-007`; `D-003A` is resolved.
...
- Last completed action: resolved `D-003A` with one compact default human diagnostic on stderr, one explicit schema-versioned JSON projection on stdout, stable prerequisite fields, bounded candidate evidence, unchanged success output, and no expected traceback.
- Current task: `AIC-004-diagnostic-design`.
- Exact next action: explain and obtain only the owner decision on `D-003B` numeric exit-code compatibility across existing entrypoints and the new native launchers.
