...
- `updated_at`: `2026-08-02T19:01:30+08:00`
...
| `D-004` | Dependency prerequisite inventory and sanctioned recovery/install operation umbrella | decomposed | Resolve exact per-entrypoint dependency profiles through `D-004A`, then recovery guidance and any explicit isolated bootstrap operation through `D-004B`. | Prevents read-only readiness scope from implicitly authorizing network, installation, or environment mutation. |
...
| `D-004B` | Recovery guidance and explicit isolated bootstrap boundary | pending |  | Determines the exact recovery command, whether repository automation may offer a separately invoked isolated environment bootstrap, and the network, filesystem, trust, cleanup, and enterprise-policy safeguards for that operation. |
...
`D-004A` authorizes only read-only readiness probes and exact missing-dependency identification. It does not authorize `pip`, `uv`, network access, environment creation, repository writes, or any other recovery mutation; those remain exclusively owned by pending `D-004B`.
...
| --- | --- | --- |
| Resolved atomic decisions (14) | `D-001`, `D-002A`, `D-002B`, `D-002C`, `D-004A`, `D-006A`, `D-006B`, `D-TR-001`, `D-010A`, `D-010B1`, `D-010B2`, `D-010C`, `D-010D`, `D-010E` | Production-entrypoint coverage, no-mutation failure, deterministic interpreter trust/discovery, stable native launchers with replaceable discovery, exact per-entrypoint dependency profiles, workflow ownership, packaged-but-unselected skill self-tests, future translation routing, routine-switch scope, local and CI policy, compatibility-safe result truth, and bounded Agent attempts are approved. |
| Decomposed umbrellas (2) | `D-002`, `D-004` | `D-002` organizes its now-resolved runtime subdecisions. `D-004A` is resolved while recovery/install behavior remains open in `D-004B`. Neither umbrella independently authorizes implementation; `D-010` is fully resolved through its control chain. |
| Pending atomic decisions (7) | `D-003`, `D-004B`, `D-005`, `D-006`, `D-007`, `D-008`, `D-009` | Output/exit contract, recovery/install behavior, pre-mutation proof, remaining script ownership, tests, migration communication, and canonical runtime remain open. |
...
1. Runtime discovery and recovery: `D-004B` and `D-009`; `D-002A`, `D-002B`, `D-002C`, and `D-004A` are resolved.
...
- Last completed action: resolved `D-004A` with Python-floor checks for all 25 production CLIs, exact PyYAML readiness for the 23 dependent CLIs, no false PyYAML gate on the two standard-library-only CLIs, and a governed dependency-profile drift check.
- Current task: `AIC-004-diagnostic-design`.
- Exact next action: explain and obtain only the owner decision on `D-004B` recovery guidance and whether a separately invoked isolated dependency bootstrap operation is offered.
