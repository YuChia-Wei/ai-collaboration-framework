...
- `updated_at`: `2026-08-02T19:36:32+08:00`
...
| --- | --- | --- |
| Resolved atomic decisions (19) | `D-001`, `D-002A`, `D-002B`, `D-002C`, `D-003A`, `D-003B`, `D-004A`, `D-004B`, `D-005`, `D-006A`, `D-006B`, `D-009`, `D-TR-001`, `D-010A`, `D-010B1`, `D-010B2`, `D-010C`, `D-010D`, `D-010E` | Production-entrypoint coverage, no-mutation failure, deterministic interpreter trust/discovery, stable native launchers with replaceable discovery, human/JSON diagnostic projections, compatibility-first exit codes, exact per-entrypoint dependency profiles, documentation-only recovery, a verifiable pre-mutation boundary, workflow ownership, packaged-but-unselected skill self-tests, retained v0.8.0 Python runtime, future translation routing, routine-switch scope, local and CI policy, compatibility-safe result truth, and bounded Agent attempts are approved. |
...
| Pending atomic decisions (3) | `D-007`, `D-006`, `D-008` | The deterministic test matrix, remaining script ownership/package compatibility, and migration communication remain open. |

At the owner's request, the final three questions are previewed together while retaining the one-question-at-a-time decision contract. Runtime discovery, recovery, disposition, and the pre-mutation boundary are fully resolved through `D-002A` to `D-002C`, `D-004A` to `D-004B`, `D-005`, and `D-009`:

1. `D-007` freezes a layered deterministic test matrix, platform coverage, portable-package checks, and required closeout gates without creating a full Cartesian product across all 25 CLIs.
2. `D-006` freezes canonical script ownership, shared prerequisite-code placement, package projection, and retained compatibility entrypoints.
3. `D-008` freezes installation, troubleshooting, command/contract migration, upgrade, and release communication.
...
- Last completed action: resolved `D-005` with bootstrap-before-domain-import ordering, bytecode suppression, bounded no-mutation surfaces, and negative-path evidence for write-capable and read-only CLIs.
- Current task: `AIC-004-diagnostic-design`.
- Exact next action: preview the final `D-007`, `D-006`, and `D-008` topics together, then explain and obtain only the owner decision on the `D-007` layered deterministic test matrix.
