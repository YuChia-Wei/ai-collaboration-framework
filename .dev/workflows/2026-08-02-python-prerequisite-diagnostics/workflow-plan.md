...
- `updated_at`: `2026-08-02T19:48:00+08:00`
...
| --- | --- | --- |
| Resolved atomic decisions (20) | `D-001`, `D-002A`, `D-002B`, `D-002C`, `D-003A`, `D-003B`, `D-004A`, `D-004B`, `D-005`, `D-006A`, `D-006B`, `D-007`, `D-009`, `D-TR-001`, `D-010A`, `D-010B1`, `D-010B2`, `D-010C`, `D-010D`, `D-010E` | Production-entrypoint coverage, no-mutation failure, deterministic interpreter trust/discovery, stable native launchers with replaceable discovery, human/JSON diagnostic projections, compatibility-first exit codes, exact per-entrypoint dependency profiles, documentation-only recovery, a verifiable pre-mutation boundary, workflow ownership, packaged-but-unselected skill self-tests, a layered Windows/Ubuntu/portable test matrix, retained v0.8.0 Python runtime, future translation routing, routine-switch scope, local and CI policy, compatibility-safe result truth, and bounded Agent attempts are approved. |
...
| Pending atomic decisions (2) | `D-006`, `D-008` | Remaining script ownership/package compatibility and migration communication remain open. |

The requested final-three preview was provided before resolving `D-007`. Two questions now remain under the one-question-at-a-time decision contract:

1. `D-006` freezes canonical script ownership, shared prerequisite-code placement, package projection, and retained compatibility entrypoints.
2. `D-008` freezes installation, troubleshooting, command/contract migration, upgrade, and release communication.
...
- Last completed action: resolved `D-007` with a layered deterministic prerequisite matrix, focused Windows and Ubuntu source-CI contracts, complete 25-entrypoint registry/smoke coverage, write-path no-mutation proof, and portable clean-install/upgrade gates.
- Current task: `AIC-004-diagnostic-design`.
- Exact next action: explain and obtain only the owner decision on `D-006` canonical script ownership, shared prerequisite placement, package projection, and compatibility routes.
