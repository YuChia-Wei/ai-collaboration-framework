...
- `updated_at`: `2026-09-05T09:35:34+08:00`
...
6. Separate implementation workflow completion from provider terminal operations and make explicit that `in_progress` alone is not an integration blocker.

These six stages retain distinct state: manifest identity can complete before rebind is admissible; rebind can complete before any current gate is enabled; pilot execution and observation can be disabled without removing the shared contract; review binding can change prospectively without rewriting historical evidence; package correction can be locally complete while already-installed v0.15.1 targets remain unchanged until separately authorized upgrades; completed implementation remains distinct from the later PR/provider terminal sequence.
...
| GOV017-F6 workflow `in_progress` was treated as an integration prohibition and provider terminal gates incorrectly kept completed implementation active | high | `slice-implementer` | separate the states while preserving checkpoint and repository gates | `GOV017-multi-hop-pilot` | source/portable policy parity, active skill routing, workflow lifecycle validation, and focused package projection |
...
| completed implementation -> push/PR/merge/Issue closure | `approved` | 2026-09-05 owner message authorizing PR, merge, and Issue closure after successful gates | current-head review binding, hosted checks, live admission, integration, and read-back |
| Issue closure -> Project mutation/release/publication | `awaiting-approval` | Project automation may update status, but no manual Project or release mutation was authorized | preserve live provider outcome without inferring additional authority |
...
- Validate that source and portable workflow policies plus active skills treat `in_progress` as non-blocking for separately authorized integration while retaining checkpoint, review, check, permission, and branch-protection gates.
...
| GOV017-A11 | passed | `in_progress` is no longer treated as an integration blocker in source policy, portable projection, or active skill routing |

## Completion Checkpoint

- Last completed action: closed the implementation workflow after validating the owner-approved content-addressed evidence model, downstream package correction, and non-blocking workflow-status boundary.
- Current task: none; `GOV017-multi-hop-pilot` is completed.
- Exact next action: perform the separately authorized PR, current-head review binding, hosted checks, live admission, merge, and Issue read-back sequence. These are provider terminal operations, not an unfinished workflow implementation task.
- Validation already completed: the prior subject tests passed 8/8, registry tests passed 10/10, and external invocation `20260903T031341Z-829` passed in 487s with 48 selected, 47 executed, zero reused, failed, or blocked checks. The content-review and portable-boundary correction additionally passed 57 terminal-closure, 13 lifecycle, 20 provider, 8 subject-digest, 9 dependency-observation, 29 guardrail, 10 language-parity, 16 wrapper-metadata, 8 governance-routing, 31 sub-agent-adapter, 10 profile-registry, and focused package-projection tests plus the relevant static validators. The workflow-status correction then passed 16 lifecycle, 22 handoff, 6 delivery-policy, 57 terminal-closure, 10 language-policy, 16 wrapper-metadata, 7 governance-workflow, and the focused downstream projection regression; AI-context and 107-workflow static validation also passed. Sandbox-only Windows Temp denials were reproduced as environment failures and the same affected suites passed outside the sandbox. The prior baseline subject digest is `aeeeba52f1504dff9fdb430fc8a631c3a045a4dca939803b0623b83ca8f2a69e`; its sealed-manifest SHA-256 is `a4afc342c32576f1797564d253a29d85e2becadf3470d4aa2f88d1f846f92b57` and is not relabeled as execution of the correction.
- Git state: at this completion checkpoint the branch remains local-only and is prepared for the now-authorized push/PR sequence. Current-head review/admission receipts remain only under ignored validation artifacts or provider records and will not produce a tracked evidence-sync commit.
...
- Blockers or unresolved decisions: no workflow-owned implementation blocker remains. Observation remains 1/3 and expansion is unauthorized. Provider terminal gates must still pass before merge/Issue closure, and already-installed v0.15.1 downstreams remain affected until a fixed release exists and each target upgrade is separately authorized.
...
| 1 | `codex/2026-09-03-subject-digest-evidence-reuse-implementation` | `45222964a2234744b0c7c7fef5add8725f985a1a` | downstream SHA-boundary correction | `0e0b7bd2df36bb9f96b8222bf89ebf49fab40c90` | local only | `2026-09-05T00:12:11+08:00` | remove SHA-only invalidation from the portable candidate and prove target-scoped projection | close the completed implementation workflow and begin the authorized provider terminal sequence |
