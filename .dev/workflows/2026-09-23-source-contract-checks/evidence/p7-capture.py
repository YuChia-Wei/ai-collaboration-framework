...
output = REPO / '.dev/ai-context/local' / ('p7-368-' + uuid.uuid4().hex)
output.mkdir()
...
try:
    ...
    plan = json.loads((WF / 'p7-reconciliation.json').read_text(encoding='utf-8'))
    assert git('rev-parse', 'HEAD:src').decode().strip() == plan['source_tree_oid']
    record['selected_source_commit'] = plan['selected_source_commit']
    record['source_tree_oid'] = plan['source_tree_oid']
