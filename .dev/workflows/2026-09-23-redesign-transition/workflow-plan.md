# Temporary Redesign Governance Workflow
...
- `branch`: `codex/2026-09-23-redesign-transition`
...
implements P0 under [program #322](https://github.com/YuChia-Wei/ai-collaboration-framework/issues/322).
The explicit authority is [execution-plan.md, U001](../../assessments/ASM-20260923-00-6oq/execution-plan.md)
and the coordinator's [issue handoff](../2026-09-23-framework-redesign-control/handoffs/issue-324.yaml).
The assigned starting commit is newer than the handoff's historical subject; the
explicit dispatch pins `53c9c8e58615e87e36f7b74ea8851fe845312daa` for this work.
...
One issue-owned task is intentional: this workflow preserves U001's exceptional
verification disposition and the local-delivery/coordinator-integration handoff.
It does not pad the implementation with separate validation or transport tasks.
The executor is user-declared `gpt-6-astra` / `ultra`; no sub-agents or additional
conversations may be created.

Only the scope files declared in [ISSUE-324.json](tasks/ISSUE-324.json) may change.
The coordinator owns the shared workflow index, its workflow and online
integration. Do not alter assessment/design files, other worktrees,
`F:/ai-context-tests`, pipeline settings, credentials, release state or downstream
artifacts. No push, PR, merge or Issue closure belongs to this task.
...
1. Bootstrap: read authoritative inputs, verify clean assigned worktree and commit this bounded execution record.
2. P0 implementation: author override and minimal pointers; retain CI/cleanup evidence references and P7 restoration criteria.
3. Local delivery: inspect actual changes, parse documents, check Git diff and planned commit message, then commit coherent result.
4. Independent post-remediation audit, legacy validators/tests, package/upgrade/migration trials and hosted required contexts: `deferred-by-owner` until P7 under U001.
5. Coordinator integration: report local HEAD and suggested index row; coordinator owns commit organization, first push, PR, online merge and provider reconciliation.

## Acceptance

| ID | Observable local completion criterion |
| --- | --- |
| AC1 | Root entry discovers U001, source scope, explicit Astra Ultra, no sub-agents and coordinator handoff without hidden history. |
| AC2 | Override limits exceptions to #322; preserves security, ownership, credentials and publication boundaries. |
| AC3 | Saved CI/cleanup evidence is referenced with provenance; all seven pipelines and restoration review/exit criteria are durable; no CI or validator execution in #324. |
| AC4 | English/Traditional Chinese entry changes align; only necessary source policy references change. |
| AC5 | Workflow distinguishes actual lightweight checks from `deferred-by-owner` verification and stops after local coherent commits. |
