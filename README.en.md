...
| Explore target-selected .NET mechanical-validation recipes | [`.ai/assets/tech-stacks/dotnet-backend/tooling/on-demand-mechanical-validation/`](.ai/assets/tech-stacks/dotnet-backend/tooling/on-demand-mechanical-validation/) |
...
The framework supplies no compilable .NET provider, project, or SDK pin by default. If a target needs a Roslyn analyzer or projection-registration test, its owner explicitly selects, creates, and verifies it from the [on-demand recipes](.ai/assets/tech-stacks/dotnet-backend/tooling/on-demand-mechanical-validation/). The DBA1001-DBA1017 mapping, severity example, and bounded snippets are `reference-only`; canonical standards remain the semantic owners.

The target owns its SDK, target framework, Roslyn and test-package versions, project wiring, severity, CI, compatibility, and fresh evidence. Recipe presence does not activate a capability or create a .NET SDK dependency for the framework release.
